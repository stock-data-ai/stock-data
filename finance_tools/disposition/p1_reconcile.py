...
_PUNISH_MARKET = {"市": "TWSE", "櫃": "TPEx", "興": "ESB"}
...
def export_punishments(as_of=None):
    ...
    for r in sorted(_punish_rows(d, stocks_only=False),
        ...
        rows.append({
            ...
            # 興櫃要明確對到 ESB：原本只判「櫃」，其餘一律 TWSE，興櫃處置股被標成上市。
            # 查不到市場（空字串）維持舊行為歸 TWSE。stock_map 端 market 只存不判斷，新增值安全。
            "market": _PUNISH_MARKET.get(r.get("market"), "TWSE"),
        })
